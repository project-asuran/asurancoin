...
const char * const BITCOIN_CONF_FILENAME = "asurancoin.conf";
...
#else
    const char* pszModule = "asurancoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Asurancoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Asurancoin";
#else
    // Unix-like
    return pathRet / ".asurancoin";
#endif
#endif
}
