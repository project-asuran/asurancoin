...
The Asurancoin Core daemon can be configured to act as a trusted "border
router", implementing the asurancoin wire protocol and relay, making
...
The ZeroMQ feature in Asurancoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building asurancoind:
...
For instance:
    ...
    $ asurancoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/asurancoind.tx.raw \
...
These options can also be provided in asurancoin.conf.
...
From the perspective of asurancoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into asurancoind directly. Furthermore, no information is
...
using. Asurancoind appends an up-counting sequence number to each
